...
{
    public class PphaFormManWorkerValidator : AbstractValidator<PphaFormManWorkerDto>
    {
        public PphaFormManWorkerValidator()
        {
            ...
            RuleFor(x => x.StartDate).NotNull().WithMessage("Proposed Start date is required");
        }

        public Func<object, string, Task<IEnumerable<string>>> ValidateValue => async (model, property) =>
        {
            var ctx = ValidationContext<PphaFormManWorkerDto>.CreateWithOptions((PphaFormManWorkerDto)model, x => x.IncludeProperties(property));
